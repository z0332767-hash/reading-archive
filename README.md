...
`Windows x64` · `0.7.1 预览版` · `本地优先` · `个人独立项目`
...
**仓库现已公开。** 当前安装包通过 GitHub Actions 提供，下载时可能需要登录 GitHub。构建产物保留 30 天；首版未签名，尚需 Windows 交互实机验收，暂不含自动更新。当前先分享 Windows 预览版；小程序开发暂缓，手机与 PC 云端同步尚未实现。档案保存在设备本地。

旧版用户请先导出 JSON，再在桌面版导入。详细步骤见 [安装与迁移](docs/GUIDE.md)。

## 豆瓣连接实验

0.7.1 新增桌面菜单「阅读档案 → 豆瓣连接验证（实验）」，用于在本机登录后验证两页观看记录读取。尚未实现完整影视导入与自动同步，使用步骤见 [豆瓣验证说明](docs/DOUBAN.md)。
